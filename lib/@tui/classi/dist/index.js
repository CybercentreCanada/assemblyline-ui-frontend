...
const AppClassificationBase = ({ variant, state, full, short_text_en, short_text_fr, long_text_en, long_text_fr, color, mx }) => {
	const theme = useTheme();
	const errorColor = useAppColor("red", 700, 300);
	const classiColor = useAppColor(color, 700, 300);
	const isLoading = state === "loading";
	const _isXlDown = useMediaQuery((theme$1) => theme$1.breakpoints.down("xl"));
	...
	const text = useMemo(() => isXlDown ? isEN() ? short_text_en : short_text_fr : isEN() ? long_text_en : long_text_fr, [
		isXlDown,
		isEN,
		short_text_en,
		short_text_fr,
	...
	return /* @__PURE__ */ jsx(Chip, {
		...
		icon: isXlDown ? null : /* @__PURE__ */ jsx(Security, {
			color: "inherit",
			sx: sxProps.icon
		}),
		variant: "outlined",
		label: {
			classi: text,
			unsupported: isXlDown ? t("classification.state.unsupported.short").toUpperCase() : t("classification.state.unsupported.long").toUpperCase(),
			error: isXlDown ? t("classification.state.error.short").toUpperCase() : t("classification.state.error.long").toUpperCase(),
			loading: /* @__PURE__ */ jsx(Skeleton, {
				variant: "text",
				animation: "wave",
				sx: { minWidth: isXlDown ? 48 : 100 },
				children: /* @__PURE__ */ jsx(Typography, { children: isXlDown ? t("classification.state.loading.short").toUpperCase() : t("classification.state.loading.long").toUpperCase() })
			})
		}[renderer]
	});
};
...
const AppClassification = ({ variant, full, overwrite, mx }) => {
