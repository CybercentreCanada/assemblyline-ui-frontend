					...
					children: [/* @__PURE__ */ jsx(List, {
						...
						children: [
							...
							user.is_admin && /* @__PURE__ */ jsxs(Fragment, { children: [
								renderSlot("topnav.profile.slots.admin.before", topnav.profile?.slots?.admin?.before),
								renderMenu("adminmenu", topnav.profile?.menus?.admin?.slot, topnav.profile?.menus?.admin?.title, topnav.profile?.menus?.admin?.i18nKey),
								renderSlot("topnav.profile.slots.admin.after", topnav.profile?.slots?.admin?.after)
							] }),
...
const AppLeftNavProvider = ({ children }) => {
	...
	const _menus = useMemo(() => {
		if (leftnav.menus) return applyMenuState(leftnav.menus, extractMenuState(menus));
		return menus || [];
	}, [menus, leftnav.menus]);
	const toggle = useCallback(() => {
