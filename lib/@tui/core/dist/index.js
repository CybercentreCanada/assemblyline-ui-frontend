...
import { AppBar, Avatar, Backdrop, Box, Breadcrumbs, Button, Card, Chip, CircularProgress, ClickAwayListener, Collapse, CssBaseline, Dialog, DialogContent, DialogTitle, Divider, Drawer, Fade, FormControl, IconButton, InputAdornment, InputBase, InputLabel, Link, List, ListItem, ListItemButton, ListItemIcon, ListItemText, ListSubheader, MenuItem, MenuList, Paper, Popper, Select, Skeleton, Slide, SnackbarContent, Stack, Switch, ThemeProvider, Toolbar, Tooltip, Typography, alpha, colors, createTheme, darken, emphasize, styled, useMediaQuery, useScrollTrigger, useTheme } from "@mui/material";
import { Trans, useTranslation } from "react-i18next";
import { cloneDeep, merge } from "lodash-es";
import { Apps, Bolt, CheckCircle, ChevronRight, Circle, Clear, Close, DarkMode, DragIndicator, Error as Error$1, Fullscreen, FullscreenExit, Info, KeyboardCommandKey, Language, Menu, Search, Tune, TurnRight, Warning, ZoomInMap, ZoomOutMap } from "@mui/icons-material";
import { getOverlayAlpha } from "@mui/material/styles";
...
const parseTuiCookies = (cookies, defaults) => {
	return parseCookies(cookies, {
		...
		key: "tui.leftNavHover",
		name: "leftNavHover",
		type: "boolean",
		default: defaultValue(defaults?.leftNavHover, AppDefaultsCookieConfigs.leftNavHover)
	}, {
...
const createCookieStore = (cookies, _i18n) => {
	const store = create((set) => {
		return {
			...
			setLeftNavHover: (hover) => {
				setClientCookie("tui.leftNavHover", `${hover}`);
				set({ leftNavHover: hover });
			},
...
const useAppPreferences = () => {
	...
	return useMemo(() => {
		...
		return {
			allowPersonalization: _configs.preferences.allowAutoHideTopbar || _configs.preferences.allowBreadcrumbs || _configs.preferences.allowQuickSearch || _configs.preferences.allowReset || _configs.preferences.allowThemeSelection || _configs.preferences.allowLayoutSelection || _configs.preferences.allowLeftNavHover,
...
const getDensityThemeOverrides = (density) => {
	...
	return {
		...
		components: {
			...
			MuiToggleButtonGroup: { defaultProps: { size } },
			...
			MuiPagination: { defaultProps: { size } },
			MuiPaginationItem: { defaultProps: { size } },
...
/**
* Baseline MUI `ThemeOptions` applied to every theme produced by
* {@link useAppThemeBuilder} before any application or density overrides.
*
* - Registers the TUI-specific `zIndex.tui.superOverlay` slot used by overlay components.
* - Forces `html`, `body`, and `#root` to fill the viewport so the app shell can rely on a
*   full-height layout container.
* - Removes the right border on `MuiDrawer` paper so the left navigation blends with the
*   surrounding surface.
*
* This is merged first in the precedence chain:
* `BASE_THEME_CONFIG` → density overrides → theme `global` → `globalOverrides` →
* theme `light`/`dark` → `light`/`dark` overrides.
*/
...
const useAppThemeBuilder = () => {
	return useCallback((theme, optionsOverride, density = "comfortable", localization = []) => {
		...
		return {
			lightTheme: createTheme(merge({}, baseConfigs, densityOverrides, global || null, globalOverrides || null, light || null, lightOverrides || null), ...localization),
			darkTheme: createTheme(merge({}, baseConfigs, densityOverrides, global || null, globalOverrides || null, dark || null, darkOverrides || null), ...localization)
		};
	}, []);
};
...
const AppThemesProvider = ({ initTheme, themes, localization, children }) => {
	...
	const { lightTheme, darkTheme } = useMemo(() => themeBuilder(current, optionsOverride, densityCookie, localization), [
		themeBuilder,
		current,
		optionsOverride,
		densityCookie,
...
//#endregion
//#region src/leftnav/rail.ts
const railWidth = (theme) => `max(${theme.spacing(7)}, 42px)`;
const floatBreakpoint = "lg";
const modalBreakpoint = "md";
const overlayWidth = (theme) => `min(320px, calc(100vw - ${theme.spacing(7)}))`;
...
//#endregion
//#region src/surface/Surface.tsx
/** MUI only defines shadows and dark-mode overlays up to 24. */
const MAX_ELEVATION = 24;
/** Past 24 MUI's overlay curve is flat, so the tint has to keep climbing linearly to stay visible. */
const OVERFLOW_OVERLAY_ALPHA_PER_STEP = .02;
const OVERFLOW_DARKEN_PER_STEP = .03;
const MAX_OVERFLOW_DARKEN = .16;
const SURFACE_ELEVATION_STEP = 2;
const SurfaceElevationContext = createContext(null);
/** Elevation of the nearest enclosing surface, or null when there is none. */
const useSurfaceElevation = () => useContext(SurfaceElevationContext);
/**
...
*/
const useResolvedSurfaceElevation = (elevation, step = SURFACE_ELEVATION_STEP) => {
	const parentElevation = useSurfaceElevation();
	return Math.max(elevation ?? (parentElevation ?? 0) + step, 0);
};
/** Publishes an elevation to descendant surfaces without rendering anything of its own. */
const SurfaceProvider = ({ children, elevation }) => /* @__PURE__ */ jsx(SurfaceElevationContext.Provider, {
	value: elevation,
	children
});
const Surface = ({ children, elevation, step, sx,...props }) => {
	const resolvedElevation = useResolvedSurfaceElevation(elevation, step);
	const overflow = Math.max(resolvedElevation - MAX_ELEVATION, 0);
	return /* @__PURE__ */ jsx(SurfaceElevationContext.Provider, {
		value: resolvedElevation,
		children: /* @__PURE__ */ jsx(Paper, {
			...props,
			elevation: Math.min(resolvedElevation, MAX_ELEVATION),
			sx: [overflow > 0 && ((theme) => {
				if (theme.palette.mode === "dark") {
					const overlay = Math.min(getOverlayAlpha(MAX_ELEVATION) + overflow * OVERFLOW_OVERLAY_ALPHA_PER_STEP, 1);
					const tint = alpha(theme.palette.common.white, overlay);
					return { backgroundImage: `linear-gradient(${tint}, ${tint})` };
				}
				return { backgroundColor: darken(theme.palette.background.paper, Math.min(overflow * OVERFLOW_DARKEN_PER_STEP, MAX_OVERFLOW_DARKEN)) };
...
//#endregion
//#region src/surface/GlassSurface.tsx
/** Fill alpha and blur move together — a thinner fill needs less blur or the content behind it turns to mush. */
const MATERIALS = {
	regular: {
		blur: 24,
		dark: .55,
...
const GlassSurface = ({ children, disableBorder = false, material = "regular", sx,...props }) => /* @__PURE__ */ jsx(Surface, {
	sx: [(theme) => {
		const isDark = theme.palette.mode === "dark";
		const rim = disableBorder ? "none" : `1px solid ${theme.palette.divider}`;
		const materialConfig = MATERIALS[material];
		if (!materialConfig) return {
			background: "transparent",
			border: rim
		};
		const filter = `blur(${materialConfig.blur}px) saturate(${materialConfig.saturate}%)`;
		...
		return {
			...
			backgroundColor: alpha(theme.palette.background.paper, isDark ? materialConfig.dark : materialConfig.light),
			border: rim,
			"@supports not ((backdrop-filter: blur(1px)) or (-webkit-backdrop-filter: blur(1px)))": opaqueFallback,
			"@media (prefers-reduced-transparency: reduce)": opaqueFallback
		};
	}, ...Array.isArray(sx) ? sx : [sx]],
	...props,
	children
...
const AppCommands = ({ open: open$1, setOpen }) => {
	...
	return /* @__PURE__ */ jsxs(Dialog, {
		...
		children: [/* @__PURE__ */ jsx(DialogTitle, { children: /* @__PURE__ */ jsx(SurfaceProvider, {
			elevation: 24,
			...
			children: [
				(i18nKey || label) && /* @__PURE__ */ jsx(Stack, {
					...
					children: /* @__PURE__ */ jsx(LeftNavAction, {
						...
						icon,
						level: _level - 1,
						activeParent: Boolean((props.matcher || props.route) && matcher(props.route ?? "", {
...
//#endregion
//#region src/skeleton/AppLeftNavElementSkeleton.tsx
const AppLeftNavElementSkeleton = ({ style, withText,...boxProps }) => {
	const theme = useTheme();
...
//#endregion
//#region src/skeleton/AppLeftNavSkeleton.tsx
const StyledLeftNav = styled("div")(({ theme }) => ({
...
const AppLeftNavElements = ({ elements, withText }) => {
	const theme = useTheme();
	return /* @__PURE__ */ jsx(Fragment, { children: elements.map((element, i) => {
		...
		return /* @__PURE__ */ jsx(AppLeftNavElementSkeleton, {
			withText,
			id: element.id,
			style: {
...
const AppLeftNavSkeleton = ({ withHeader = false }) => {
	const leftnav = useAppLeftNav();
	const { leftnav: leftnavPreference } = useAppPreferences();
	const theme = useTheme();
	const sp1 = theme.spacing(1);
	const sp2 = theme.spacing(2);
	const width = leftnav.open ? leftnavPreference.width : railWidth(theme);
	const widthCss = typeof width === "number" ? `${width}px` : width;
	return /* @__PURE__ */ jsxs(StyledLeftNav, {
		style: { "--left-nav-width": widthCss },
		children: [withHeader && /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsx(Toolbar, {
			disableGutters: true,
			children: /* @__PURE__ */ jsx(AppLeftNavElementSkeleton, {
		...
		}), /* @__PURE__ */ jsx(Divider, {})] }), /* @__PURE__ */ jsx(List, {
			disablePadding: true,
			sx: {
				display: "flex",
				flexDirection: "column",
			...
			children: /* @__PURE__ */ jsx(AppLeftNavElements, {
...
const TuiBreadcrumb = ({ path, icon, i18nKey, title, width }) => {
	...
	return /* @__PURE__ */ jsxs(Fragment, { children: [icon && /* @__PURE__ */ jsxs(Fragment, { children: [icon, /* @__PURE__ */ jsx(Box, {
		component: "span",
		sx: { mr: 1 }
	})] }), /* @__PURE__ */ jsx("span", {
		style: {
...
const AppName = ({ noName }) => {
	...
	if (isXs) return /* @__PURE__ */ jsxs(StyledTitle, { children: [/* @__PURE__ */ jsx(Tooltip, {
...
const SWITCH_GUTTER = 48;
const ellipsisSx = { "& .MuiListItemText-primary": {
	whiteSpace: "nowrap",
	overflow: "hidden",
	textOverflow: "ellipsis"
} };
const switchListSx = (theme) => ({
	...ellipsisSx,
	"& .MuiListItem-root > .MuiListItemButton-root": { pr: `max(${theme.spacing(8)}, ${SWITCH_GUTTER}px)` }
});
const ThemeSelection = () => {
	...
	const leftNavHover = useCookiesStore((store) => store.leftNavHover);
	const setLeftNavHover = useCookiesStore((store) => store.setLeftNavHover);
	...
	const { allowTranslate, allowPersonalization, allowLayoutSelection, allowQuickSearch, allowBreadcrumbs, allowAutoHideTopbar, allowReset, allowThemeSelection, allowDensitySelection, allowLeftNavHover } = useAppPreferences();
	return /* @__PURE__ */ jsxs("div", { children: [
		...
		allowPersonalization && /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsx(Divider, {}), /* @__PURE__ */ jsxs(List, {
			...
			children: [
				...
				allowLeftNavHover && !isSmDown && /* @__PURE__ */ jsx(ListItem, {
					disablePadding: true,
					secondaryAction: /* @__PURE__ */ jsx(Switch, {
						edge: "end",
						checked: leftNavHover,
						onClick: () => setLeftNavHover(!leftNavHover)
					}),
					children: /* @__PURE__ */ jsx(ListItemButton, {
						onClick: () => setLeftNavHover(!leftNavHover),
						id: "personalization-leftnavhover",
						children: /* @__PURE__ */ jsx(ListItemText, { children: t("personalization.leftnavhover") })
					})
				}),
...
const UserProfile = () => {
	...
	const [open$1, setOpen] = useState(false);
	const hasContentBeforeUserMenu = Boolean(topnav.profile?.slots?.top?.length) || allowFocusMode || Boolean(topnav.profile?.slots?.userMenu?.before?.length);
	const renderSlot = useCallback((id, slot) => {
		return /* @__PURE__ */ jsx(OverlayShadow, {
			region: "slot",
			id,
			children: slot?.length ? /* @__PURE__ */ jsx(Stack, {
				direction: "column",
				spacing: 1,
				children: slot
			}) : null
	...
	const renderFocusMode = useCallback(() => {
		if (!allowFocusMode) return null;
	...
	const renderMenu = useCallback((type, menuItems, title, i18nKey, withDivider = true, dividerTopSpacing = false) => {
		if (menuItems !== void 0 && menuItems !== null && menuItems.length !== 0) return /* @__PURE__ */ jsxs("div", { children: [withDivider && /* @__PURE__ */ jsx(Divider, { sx: dividerTopSpacing ? { mt: 1 } : void 0 }), /* @__PURE__ */ jsx(List, {
			dense: true,
			subheader: /* @__PURE__ */ jsx(ListSubheader, {
				disableSticky: true,
				children: i18nKey ? clientT(i18nKey) : title
			}),
			children: menuItems.map((a, i) => a.element ? /* @__PURE__ */ jsx(ListItem, { children: a.element }, `${type}-${i}`) : /* @__PURE__ */ jsxs(ListItemButton, {
				component: Link$1,
				to: a.route,
			...
			children: ({ TransitionProps }) => /* @__PURE__ */ jsx(Fade, {
				...
				children: /* @__PURE__ */ jsxs(Paper, {
					...
					children: [/* @__PURE__ */ jsx(List, {
						...
						children: [
							renderSlot("topnav.profile.slots.top", topnav.profile?.slots?.top),
							renderFocusMode(),
							renderSlot("topnav.profile.slots.userMenu.before", topnav.profile?.slots?.userMenu?.before),
							renderMenu("usermenu", topnav.profile?.menus?.user?.slot, topnav.profile?.menus?.user?.title, topnav.profile?.menus?.user?.i18nKey, hasContentBeforeUserMenu, hasContentBeforeUserMenu),
							renderSlot("topnav.profile.slots.userMenu.after", topnav.profile?.slots?.userMenu?.after),
							user.is_admin && /* @__PURE__ */ jsxs(Fragment, { children: [
								renderSlot("topnav.profile.slots.adminMenu.before", topnav.profile?.slots?.adminMenu?.before),
								renderMenu("adminmenu", topnav.profile?.menus?.admin?.slot, topnav.profile?.menus?.admin?.title, topnav.profile?.menus?.admin?.i18nKey),
								renderSlot("topnav.profile.slots.adminMenu.after", topnav.profile?.slots?.adminMenu?.after)
							] }),
							renderThemeSelection(topnav.themeSelectionMode === "profile"),
							renderSlot("topnav.profile.slots.bottom", topnav.profile?.slots?.bottom)
						]
					})]
				})
			})
...
const AppBarBase = ({ children }) => {
	...
	return /* @__PURE__ */ jsx(AppBar, {
		...
		sx: (theme) => ({
			...
			color: theme.palette.text.primary,
			backgroundColor: isTopLayout ? theme.palette.background.paper : theme.palette.background.default,
...
const AppBarSkeleton = ({ layout }) => {
	...
	return /* @__PURE__ */ jsx(AppBarBase, { children: /* @__PURE__ */ jsxs(Toolbar, {
		...
		children: [
			isXs && /* @__PURE__ */ jsx(AppLeftNavElementSkeleton, {
				withText: leftnav.open,
				style: {
					paddingTop: sp1,
					paddingBottom: sp1,
			...
			layout === "top" && /* @__PURE__ */ jsxs(Fragment, { children: [!isXs && /* @__PURE__ */ jsx(AppLeftNavElementSkeleton, {
				withText: false,
				style: {
					paddingTop: sp1,
					paddingBottom: sp1,
			...
			topnavPreferences.slots?.left?.length > 0 && /* @__PURE__ */ jsx(Stack, {
				...
				spacing: 3,
				mr: 3,
				children: topnavPreferences.slots.left.map((_, index) => /* @__PURE__ */ jsx(AppLeftNavElementSkeleton, {
					withText: false,
					style: {}
			...
			topnavPreferences.slots?.breadcrumbs?.left?.length > 0 && /* @__PURE__ */ jsx(Stack, {
				...
				spacing: 3,
				mr: 3,
				children: topnavPreferences.slots.breadcrumbs.left.map((_, index) => /* @__PURE__ */ jsx(AppLeftNavElementSkeleton, {
					withText: false,
					style: {}
			...
			topnavPreferences.slots?.breadcrumbs?.right?.length > 0 && /* @__PURE__ */ jsx(Stack, {
				...
				spacing: 3,
				mr: 3,
				children: topnavPreferences.slots.breadcrumbs.right.map((_, index) => /* @__PURE__ */ jsx(AppLeftNavElementSkeleton, {
					withText: false,
					style: {}
			...
			topnavPreferences.slots?.search?.left?.length > 0 && /* @__PURE__ */ jsx(Stack, {
				...
				spacing: 3,
				ml: 3,
				children: topnavPreferences.slots.search.left.map((_, index) => /* @__PURE__ */ jsx(AppLeftNavElementSkeleton, {
					withText: false,
					style: {}
				}, `topnav.slots.search.left.${index}`))
			}),
			quicksearch.show && (isSmDown || topnavPreferences.quickSearchIconOnly ? /* @__PURE__ */ jsx(AppLeftNavElementSkeleton, {
				withText: false,
				style: {}
			...
			topnavPreferences.slots?.search?.right?.length > 0 && /* @__PURE__ */ jsx(Stack, {
				...
				spacing: 3,
				ml: 3,
				children: topnavPreferences.slots.search.right.map((_, index) => /* @__PURE__ */ jsx(AppLeftNavElementSkeleton, {
					withText: false,
					style: {}
			...
			topnavPreferences.slots?.right?.length > 0 && /* @__PURE__ */ jsx(Stack, {
				...
				spacing: 3,
				ml: 3,
				children: topnavPreferences.slots.right.map((_, index) => /* @__PURE__ */ jsx(AppLeftNavElementSkeleton, {
					withText: false,
					style: {}
...
const SideLayoutSkeleton = () => {
	return /* @__PURE__ */ jsx(StyledContainer, { children: /* @__PURE__ */ jsx(StyledLeftLayout, { children: /* @__PURE__ */ jsxs(StyledContent, { children: [/* @__PURE__ */ jsx(AppLeftNavSkeleton, { withHeader: true }), /* @__PURE__ */ jsx(StyledContentRight, { children: /* @__PURE__ */ jsx(AppBarSkeleton, { layout: "side" }) })] }) }) });
};
/**
* A Skeleton for the top layout.
*/
const TopLayoutSkeleton = () => {
	return /* @__PURE__ */ jsx(StyledContainer, { children: /* @__PURE__ */ jsxs(StyledTopLayout, { children: [/* @__PURE__ */ jsx(AppBarSkeleton, { layout: "top" }), /* @__PURE__ */ jsxs(StyledContent, { children: [/* @__PURE__ */ jsx(AppLeftNavSkeleton, {}), /* @__PURE__ */ jsx(StyledContentRight, {})] })] }) });
};
/**
...
//#endregion
//#region src/leftnav/LeftNavScrollArea.tsx
const THUMB_MIN_HEIGHT = 24;
const TRACK_INSET = 4;
const THUMB_CLASS = "LeftNavScrollArea-thumb";
const CUE_CLASS = "LeftNavScrollArea-cue";
const NO_FADE = {
	top: false,
	bottom: false
};
...
const FADE_CLEAR = 2;
const FADE_DEPTH = 6;
/** Points at content the edge fade cannot advertise on its own, because a fade needs something under it to act on. */
const ScrollCue = ({ edge, hidden }) => /* @__PURE__ */ jsx(Box, {
	className: CUE_CLASS,
	sx: (theme) => {
		const half = theme.spacing(.75);
		return {
			position: "absolute",
			[edge]: CUE_INSET,
			left: "50%",
			...
			width: 0,
			height: 0,
			borderLeft: `${half} solid transparent`,
			borderRight: `${half} solid transparent`,
			...edge === "top" ? { borderBottom: `${half} solid ${theme.palette.text.primary}` } : { borderTop: `${half} solid ${theme.palette.text.primary}` },
			pointerEvents: "none",
			opacity: hidden ? 0 : 1,
...
/**
* Scroll container for the left nav menus with a thumb painted over the content instead of beside it.
*
* The native scrollbar is suppressed because it reserves layout width, which the collapsed rail cannot spare
* without shifting the icons off centre.
*/
const LeftNavScrollArea = ({ children }) => {
	...
	const measure = useCallback(() => {
		...
		const height = Math.max(THUMB_MIN_HEIGHT, clientHeight / scrollHeight * trackHeight);
		setThumb({
			top: TRACK_INSET + scrollTop / overflow * Math.max(0, trackHeight - height),
			height
		});
		setFade({
			top: scrollTop > 1,
			bottom: scrollTop < overflow - 1
		});
	}, []);
	useEffect(() => {
		const el = scrollRef.current;
	...
	const onPointerMove = useCallback((event) => {
		...
		const track = el.clientHeight - TRACK_INSET * 2 - thumb.height;
		if (track <= 0) return;
		el.scrollTop = drag.scrollTop + (event.clientY - drag.pointerY) * ((el.scrollHeight - el.clientHeight) / track);
	}, [thumb]);
	...
	return /* @__PURE__ */ jsxs(Box, {
		sx: {
			...
			flexDirection: "column",
			minHeight: 0,
			[`&:hover .${THUMB_CLASS}`]: { opacity: 1 },
			[`&:hover .${CUE_CLASS}`]: { opacity: 0 }
		},
		children: [
			/* @__PURE__ */ jsx(Stack, {
				ref: scrollRef,
				onScroll: measure,
				sx: (theme) => {
					const stop = (on, units) => on ? theme.spacing(units) : "0px";
					const clearTop = stop(fade.top, FADE_CLEAR);
					const depthTop = stop(fade.top, FADE_DEPTH);
					const clearBottom = stop(fade.bottom, FADE_CLEAR);
					const depthBottom = stop(fade.bottom, FADE_DEPTH);
					return {
						overflowY: "auto",
						overflowX: "hidden",
						scrollbarWidth: "none",
						"&::-webkit-scrollbar": { display: "none" },
						maskImage: `linear-gradient(to bottom, transparent ${clearTop}, #000 ${depthTop}, #000 calc(100% - ${depthBottom}), transparent calc(100% - ${clearBottom}))`
					};
				},
				children: /* @__PURE__ */ jsx(Box, {
					ref: contentRef,
			...
			thumb && /* @__PURE__ */ jsx(Box, {
				...
				sx: (theme) => ({
					...
					"&:hover": { bgcolor: theme.palette.action.active }
				})
			}),
			fade.top && /* @__PURE__ */ jsx(ScrollCue, {
				edge: "top",
				hidden: dragging
			}),
			fade.bottom && /* @__PURE__ */ jsx(ScrollCue, {
				edge: "bottom",
				hidden: dragging
			})
		]
...
const StyledDrawer = styled(Drawer, { shouldForwardProp: (prop) => prop !== "open" && prop !== "width" })(({ theme, open: open$1, width }) => ({
	...
	...open$1 && { [theme.breakpoints.down(modalBreakpoint)]: { width: overlayWidth(theme) } },
	...!open$1 && {
		transition: theme.transitions.create("width", {
			easing: theme.transitions.easing.sharp,
		...
		overflowX: "hidden",
		width: 0,
		[theme.breakpoints.up("sm")]: { width: railWidth(theme) },
...
const LeftNavDrawer = () => {
	...
	const leftnav = useAppLeftNav();
	const { leftnav: leftnavPreference } = useAppPreferences();
	const isFloating = useMediaQuery(theme.breakpoints.down(floatBreakpoint));
	const floating$1 = leftnav.hover && leftnav.open;
	const tint = alpha(theme.palette.common.white, .06);
	const onCloseDrawerIfOpen = useCallback(() => {
		if (isFloating && leftnav.open) leftnav.setOpen(false);
	}, [isFloating, leftnav]);
	const onToggle = useCallback(() => {
		if (!leftnav.open) leftnav.collapseMenus();
		leftnav.toggle();
	...
	const onMouseEnter = useCallback(() => {
		if (leftnav.hover) leftnav.setHoverOpen(true);
	}, [leftnav]);
	const onMouseLeave = useCallback(() => {
		if (leftnav.hover) leftnav.setHoverOpen(false);
	}, [leftnav]);
	return /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsx(Backdrop, {
		open: leftnav.open && !leftnav.hover,
		sx: {
			zIndex: theme.zIndex.drawer - 1,
			[theme.breakpoints.up(modalBreakpoint)]: { display: "none" }
		}
	}), /* @__PURE__ */ jsx(ClickAwayListener, {
		...
		children: /* @__PURE__ */ jsxs(StyledDrawer, {
			...
			onMouseLeave,
			slotProps: { paper: {
				elevation: floating$1 ? 8 : 1,
				style: {
					display: "flex",
					flexDirection: "column",
					...floating$1 && theme.palette.mode === "dark" && { "--Paper-overlay": `linear-gradient(${tint}, ${tint})` }
				}
			} },
			variant: "permanent",
			style: { height: "100%" },
			...
			children: [
				...
				/* @__PURE__ */ jsx(LeftNavScrollArea, { children: /* @__PURE__ */ jsx(OverlayShadow, {
					region: "layout",
					id: "app-leftnav-menus",
					children: leftnav.loading ? /* @__PURE__ */ jsx(AppLeftNavSkeleton, {}) : /* @__PURE__ */ jsx(LeftNavMenuRoot, {})
				}) }),
				/* @__PURE__ */ jsx(Box, {
					sx: {
						flexGrow: 1,
						...!leftnav.hover && { "&:hover": { cursor: "pointer" } }
					},
					onClick: leftnav.hover ? void 0 : onToggle
				}),
				!leftnav.hover && /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsx(Divider, {}), /* @__PURE__ */ jsx(LeftNavFooter, {})] })
			]
		})
	})] });
};
...
const AppVerticalLeft = styled("div", { shouldForwardProp: (prop) => prop !== "overlay" })(({ theme, overlay }) => ({
	height: "100%",
	[theme.breakpoints.down(floatBreakpoint)]: floating,
	...overlay && floating
}));
const AppVerticalRight = styled("div", { shouldForwardProp: (prop) => prop !== "overlay" && prop !== "reserve" })(({ theme, overlay, reserve }) => ({
	"@media print": {
	...
	minWidth: 0,
	...reserve && {
		[theme.breakpoints.between("sm", floatBreakpoint)]: { paddingLeft: railWidth(theme) },
		...overlay && { [theme.breakpoints.up(floatBreakpoint)]: { paddingLeft: railWidth(theme) } }
	}
}));
...
const AppLayoutProvider = ({ children }) => {
	...
	return /* @__PURE__ */ jsxs(AppLayoutContext.Provider, {
		...
		children: [
			...
			{
				side: /* @__PURE__ */ jsxs(AppVertical, { children: [/* @__PURE__ */ jsx(AppVerticalLeft, {
					overlay: leftnav.hover,
					children: user.isReady() && ready && showNavs && /* @__PURE__ */ jsx(LeftNavDrawer_default, {})
				}), /* @__PURE__ */ jsxs(AppVerticalRight, {
					overlay: leftnav.hover,
					reserve: showNavs,
					id: "app-scrollct",
					style: { overflow: "auto" },
				...
				top: /* @__PURE__ */ jsxs(AppHorizontal, {
					id: "app-scrollct",
					style: { overflow: "auto" },
					children: [user.isReady() && ready && showNavs && /* @__PURE__ */ jsx(AppBar_default, {}), /* @__PURE__ */ jsxs(AppVertical, { children: [/* @__PURE__ */ jsx(AppVerticalLeft, {
						overlay: leftnav.hover,
						children: user.isReady() && ready && showNavs && /* @__PURE__ */ jsx(LeftNavDrawer_default, {})
					}), /* @__PURE__ */ jsx(AppVerticalRight, {
						overlay: leftnav.hover,
						reserve: showNavs,
...
const extractMenuState = (menus = []) => {
	...
	const collect = (menu) => {
		...
		menu.items.forEach((item) => {
			if (item.type === "menu") collect(item);
		});
	};
	menus.forEach(collect);
	return state;
};
const applyMenuState = (menus = [], state) => {
	return menus.map((menu) => {
		const menuState = state.get(menu.id);
		return {
			...menu,
			...menuState || {},
			items: menu.items.map((item) => item.type === "menu" ? applyMenuState([item], state)[0] : item)
		};
	});
};
const AppLeftNavProvider = ({ children }) => {
	const theme = useTheme();
	const { leftnav, allowLeftNavHover } = useAppPreferences();
	const drawerOpen = useCookiesStore((state) => state.drawerOpen);
	const setDrawerOpen = useCookiesStore((state) => state.setDrawerOpen);
	const leftNavHoverCookie = useCookiesStore((state) => state.leftNavHover);
	const isXs = useMediaQuery(theme.breakpoints.only("xs"));
	const leftNavHover = Boolean(leftNavHoverCookie && allowLeftNavHover) && !isXs;
	const [hoverOpen, setHoverOpen] = useState(false);
	const navOpen = leftNavHover ? hoverOpen : drawerOpen;
	const [menus, setMenus] = useState(leftnav.menus || []);
	const [loading, setLoading] = useState(false);
	const [syncedMenus, setSyncedMenus] = useState(leftnav.menus || []);
	const incomingMenus = leftnav.menus || [];
	if (incomingMenus.length !== syncedMenus.length || incomingMenus.some((menu, index) => menu !== syncedMenus[index])) {
		setSyncedMenus(incomingMenus);
		setMenus((current) => applyMenuState(incomingMenus, extractMenuState(current)));
	}
	const toggle = useCallback(() => {
		setDrawerOpen(!drawerOpen);
	}, [drawerOpen, setDrawerOpen]);
	const setHoverOpenState = useCallback((open$1) => {
		setHoverOpen(open$1);
	}, []);
	const updateMenu = useCallback((menuId, updater) => {
		setMenus((current) => current.map((_menu) => ({
			..._menu,
	...
	const toggleMenu = useCallback((menuId) => {
		setMenus((current) => current.map((_menu) => ({
			...
			items: visit(_menu.items, (child) => child.type === "menu" && child.id === menuId, (child) => ({
				...child,
				expanded: navOpen ? !child.expanded : child.expanded,
				popped: navOpen ? child.popped : !child.popped
			}))
		})));
	}, [navOpen]);
	const closeMenu = useCallback((menuId) => {
		setMenus((current) => current.map((_menu) => ({
			...
			items: visit(_menu.items, (child) => child.type === "menu" && child.id === menuId, (child) => ({
				...child,
				expanded: navOpen ? false : child.expanded,
				popped: navOpen ? child.popped : false
			}))
		})));
	}, [navOpen]);
	const openMenu = useCallback((menuId) => {
		setMenus((current) => current.map((_menu) => ({
			...
			items: visit(_menu.items, (child) => child.type === "menu" && child.id === menuId, (child) => ({
				...child,
				expanded: navOpen ? true : child.expanded,
				popped: navOpen ? child.popped : true
			}))
		})));
	}, [navOpen]);
	...
	const value = useMemo(() => ({
		...
		open: navOpen,
		hover: leftNavHover,
		setHoverOpen: setHoverOpenState,
		setLoading,
...
const AppProvider = ({ router, user, search, preferences, children }) => {
	...
	return /* @__PURE__ */ jsx(AppContext.Provider, {
		...
		children: /* @__PURE__ */ jsx(AppUserProvider_default, {
			service: user,
			children: /* @__PURE__ */ jsx(AppBarProvider_default, {
				search,
	...
	configs: {
		light: {
			palette: {
				...
				info: { main: ACCENT_LAYER$1.light.BLUE },
				success: { main: ACCENT_LAYER$1.light.GREEN },
				warning: { main: ACCENT_LAYER$1.light.ORANGE },
		...
		dark: {
			palette: {
				...
				info: { main: ACCENT_LAYER$1.dark.BLUE },
				success: { main: ACCENT_LAYER$1.dark.GREEN },
				warning: { main: ACCENT_LAYER$1.dark.ORANGE },
...
`;
const variantIcons = {
	success: /* @__PURE__ */ jsx(CheckCircle, { fontSize: "small" }),
	error: /* @__PURE__ */ jsx(Error$1, { fontSize: "small" }),
	warning: /* @__PURE__ */ jsx(Warning, { fontSize: "small" }),
	info: /* @__PURE__ */ jsx(Info, { fontSize: "small" })
};
const ThemedSnackbar = forwardRef(function ThemedSnackbar$1(props, ref) {
	...
	const message = icon ? /* @__PURE__ */ jsxs(Box, {
		sx: {
			display: "flex",
			alignItems: "center",
			gap: 1
		},
		children: [icon, props.message]
	}) : props.message;
...
const AppRoot = ({ i18n: i18n$1, cookies, themes, localization, children }) => {
	return /* @__PURE__ */ jsx(AppCookiesProvider, {
		cookies,
		i18n: i18n$1,
		children: /* @__PURE__ */ jsx(AppThemesProvider, {
			themes: themes ?? TUI_THEMES,
			localization,
			children: /* @__PURE__ */ jsxs(AppSnackbarProvider_default, { children: [/* @__PURE__ */ jsx(CssBaseline, { enableColorScheme: true }), children] })
		})
	});
};
...
var en_default = {
	...
	"personalization.leftnavhover": "Open left navigation on hover",
...
var fr_default = {
	...
	"personalization.leftnavhover": "Ouvrir la navigation gauche au survol",
	"personalization.quicksearch": "Afficher recherche rapide",
	"personalization.reset_text": "Réinitialiser les paramètres",
	"personalization.showbreadcrumbs": "Afficher le fil d'Ariane",
...
//#endregion
export { APPBAR_READY_EVENT, ARROW_DOWN, ARROW_LEFT, ARROW_RIGHT, ARROW_UP, AppAvatar, AppBrand, AppCookiesProvider, AppDefaultsCookieConfigs, AppDefaultsLeftNavConfigs, AppDefaultsPreferencesConfigs, AppDefaultsThemeConfigs, AppDefaultsTopNavConfigs, AppDensity, AppInfoPanel, AppLeftNavElementSkeleton, AppLeftNavSkeleton, AppListEmpty, AppProvider, AppRoot, AppThemePicker, AppThemesProvider, AppToc, AppUserAvatar, AppUserContext, BACKSPACE, BRAND_VARIANTS, ENTER, ESCAPE, GlassSurface, LayoutSkeleton, LeftNavAction, LeftNavItem, LeftNavRoute, MODULE_NAME, OverlayDefs, OverlayLabel, OverlayProvider, OverlayShadow, PageCardCentered, PageCenter, PageContent, PageFullWidth, PageFullscreen, PageHeader, SPACE, SURFACE_ELEVATION_STEP, Surface, SurfaceProvider, TUI_COOKIE_KEYS, TUI_COOKIE_OPTIONS, TUI_THEMES, addTranslations, getBrandSizes, getDensityThemeOverrides, is, isArrowDown, isArrowLeft, isArrowRight, isArrowUp, isBackspace, isEnter, isEscape, isSpace, keyboard, parseCookies, parseEvent, parseExtraServerCookie, parseTuiClientCookies, parseTuiCookies, parseTuiServerCookies, setClientCookie, traverse, useAppBar, useAppBarHeight, useAppBarScrollTrigger, useAppBrand, useAppBreadcrumbs, useAppColor, useAppDensity, useAppEnv, useAppLanguage, useAppLayout, useAppLeftNav, useAppLogo, useAppOverlay, useAppPreferences, useAppQuickSearch, useAppRouter, useAppSearchService, useAppTheme, useAppThemeBuilder, useAppUser, useClipboard, useCookiesStore, useFullscreenStatus, useLocalStorage, useLocalStorageItem, usePageProps, usePathMatcher, useResolvedSurfaceElevation, useSurfaceElevation, visit };
