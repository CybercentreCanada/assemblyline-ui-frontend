...
import { AppDensityPicker } from '../../themes/elements/AppDensityPicker';
  ...
  return (
    <div>
      ...
      {allowThemeSelection && (
        <>
          <Divider />
          <List dense subheader={<ListSubheader disableSticky>{t('personalization.theme.title')}</ListSubheader>}>
            {themes?.length > 1 && (
              <ListItem sx={{ mb: 1 }}>
                <AppThemePicker />
              </ListItem>
      ...
      {(allowPersonalization || allowThemeSelection) && allowReset && (
        <>
          <Divider />
          <List dense>
