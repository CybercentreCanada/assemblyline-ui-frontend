...
const LeftNavDrawer: React.FC<LeftNavDrawerProps> = (props) => {
  ...
  return (
    <ClickAwayListener mouseEvent="onMouseDown" touchEvent="onTouchStart" onClickAway={onCloseDrawerIfOpen} >
      ...
      >
        {drawerState ? header: <Tooltip title={layoutProps.appName} aria-label={layoutProps.appName} placement="right">{header}</Tooltip>}
        <List disablePadding>
        ...
        <Divider />
        {drawerState ? hide : <Tooltip title={t("drawer.expand")} aria-label={t("drawer.expand")} placement="right">{hide}</Tooltip>}
