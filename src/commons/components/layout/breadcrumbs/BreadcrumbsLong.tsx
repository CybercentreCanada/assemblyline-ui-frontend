...
const splitItems = (items: BreadcrumbItem[], disableEllipsis: boolean): {first: BreadcrumbItem, ellipsis: BreadcrumbItem[], after: BreadcrumbItem[], last: BreadcrumbItem} => {
    const first = items[0]
    let ellipsis = null;
...
const BreadcrumbsLong: React.FC<BreadcrumbsLongProps> = ({items, disableStyle, disableEllipsis, allLinks = false, textOnly = false, compact = false}) => {
    ...
    return<MuiBreadcrumbs separator={<NavigateNextIcon fontSize="small" />}  aria-label="breadcrumb" className={!disableStyle ? classes.breadcrumbs : ""} style={compact ? {fontSize: "0.8rem"} : null}>   
        {items.length === 1 && !allLinks ? <BreadcrumbLastItem key={`bcrumb-${first.route.path}`} item={first} textOnly={textOnly} /> : first ? <BreadcrumbLinkItem key={`bcrumb-${first.route.path}`} item={first} textOnly={textOnly} /> : null} 
        {ellipsis ? <BreadcrumbEllipsisItem key="bcrumb-ellipsis" items={ellipsis} /> : null}
        {after ? after.map(item => <BreadcrumbLinkItem key={`bcrumb-${item.route.path}`} item={item} textOnly={textOnly}  />) : null}
        {last ? allLinks ?  <BreadcrumbLinkItem key={`bcrumb-${last.route.path}`} item={last} textOnly={textOnly} /> : <BreadcrumbLastItem key={`bcrumb-${last.route.path}`} item={last} textOnly={textOnly} /> : null}
    </MuiBreadcrumbs>
}
