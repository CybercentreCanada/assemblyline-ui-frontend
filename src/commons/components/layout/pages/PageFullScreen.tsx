...
import { makeStyles, Box, IconButton, useScrollTrigger } from "@material-ui/core";
...
export default function PageFullscreen({ children }) {
  ...
  let isFullscreen, setIsFullscreen, fullscreenSupported;
  ...
  return <div ref={maximizableElement} className={classes.root}>
    <Box className={classes.page}>
      <MoveOnScroll enabled={!isTopLayout && autoHideAppbar} isFullscreen={isFullscreen}>
        { fullscreenSupported ? null : 
            isFullscreen ? <IconButton onClick={handleExitFullscreen}><FullscreenExitIcon/></IconButton> : 
                           <IconButton onClick={handleEnterFullscreen}><FullscreenIcon/></IconButton>
        }
      </MoveOnScroll>
      {children}
    </Box>
