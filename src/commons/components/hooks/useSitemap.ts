...
import { useHistory, matchPath, match } from "react-router-dom"
...
const resolveTitle = (breadcrumb: BreadcrumbItem, siteMap: SiteMapRoute[]) => {
    ...
    var title = breadcrumb.route.path ? getRoute(breadcrumb.route.path, siteMap).route.title : breadcrumb.route.title;
    // if there are any parameters in match, replace them in title.
    if(breadcrumb.matcher && breadcrumb.matcher.params) {
        const params = breadcrumb.matcher.params;
