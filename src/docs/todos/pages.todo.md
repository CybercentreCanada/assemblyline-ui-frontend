...
- [ ] **Run full pages layout alignment pass** — Review and update all pages so layout structure, spacing, and route header/breadcrumb presentation are consistent with the route metadata model (`ancestor`, `title`, `icon`) and current design baseline.
