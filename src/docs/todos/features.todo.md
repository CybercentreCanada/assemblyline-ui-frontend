...
Tasks related to `src/features/` (classification, form, path-params, portal, prop-provider, search-params, store, table-of-content).
...
- [ ] **Port `SafeResultsProvider`** — Migrate from `src/components/providers/SafeResultsProvider.tsx` to a new `src/features/safe-results/` module (or integrate into `core/preference/`). Provides:
