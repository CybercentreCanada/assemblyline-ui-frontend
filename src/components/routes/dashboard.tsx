...
function Dashboard() {
  ...
  return <>
    <PageFullscreen>
      <Typography gutterBottom={true} color="primary" variant="h2" align="center">{t('page.dashboard.title')}</Typography>
      <Box display="flex" flexDirection="row" flexWrap="wrap" width="100%">
      {cards.map((a, i) => (
        <Card key={i} className={classes.card}>
            <Typography variant="h4" className={classes.watermark}># {a.id}</Typography>
        </Card>
      ))}
      </Box>
    </PageFullscreen>
