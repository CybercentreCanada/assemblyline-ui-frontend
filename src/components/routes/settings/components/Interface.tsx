...
export const InterfaceSection = React.memo(() => {
  ...
  const form = useForm();
  const { configuration, settings, c12nDef } = useALContext();
  ...
  return (
    <form.Subscribe
      selector={state =>
        [state.values.state.customize, state.values.state.loading, state.values.state.disabled] as const
      }
    >
      {([customize, loading, disabled]) => (
        <>
