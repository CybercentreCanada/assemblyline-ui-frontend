...
const MetaList: React.FC<MetaListProps> = ({
  ...
  const computeFrame = (_buffer: MetaListBuffer, _rowHeight: number): MetaListFrame => {
    const { current: _outerEL } = outerEL;
    const { current: _innerEL } = innerEL;
    const fH = _outerEL.getBoundingClientRect().height;
    const tH = _innerEL.getBoundingClientRect().height;
    const sT = _outerEL.scrollTop;
    const cP = sT + fH;
  ...
  const onScroll = () => {
    // compute frame.
    const _frame = computeFrame(buffer, rowHeight);
