...
import { NON_PRINTABLE_COPY_TYPE_VALUES, OutlinedField, SelectField, StoreProps, useDispatch } from '../..';

export const WrappedCopyTypeSetting = ({ store }: StoreProps) => {
  const { t } = useTranslation(['hexViewer']);
  const { onSettingCopyNonPrintableTypeChange, onSettingCopyNonPrintablePrefixChange } = useDispatch();
  ...
  return (
    <>
      ...
      />
      {store.setting.copy.nonPrintable.type === 2 ? (
        <OutlinedField
          label={t('copy.nonPrintable.prefix.label')}
          description={t('copy.nonPrintable.prefix.description')}
          value={store.setting.copy.nonPrintable.prefix}
          onChange={event => onSettingCopyNonPrintablePrefixChange({ event })}
        />
      ) : (
        <></>
      )}
