...
describe('updateAppQuery', () => {
  it('updates the api_response of queries whose url starts with the request url', () => {
    QUERY_CLIENT.clear();
    QUERY_CLIENT.setQueryData(['/api/foo/1', 'GET', 'null', true], { api_response: { count: 1 } });
    ...
    expect(QUERY_CLIENT.getQueryData(['/api/foo/1', 'GET', 'null', true])).toEqual({ api_response: { count: 2 } });
  });

  it('leaves queries whose url does not match untouched', () => {
    QUERY_CLIENT.clear();
    QUERY_CLIENT.setQueryData(['/api/bar', 'GET', 'null', true], { api_response: { count: 1 } });

    updateAppQuery({ url: '/api/foo' } as never, (prev: never) => prev);

    expect(QUERY_CLIENT.getQueryData(['/api/bar', 'GET', 'null', true])).toEqual({ api_response: { count: 1 } });
  });
  ...
  it('matches on method and body subset like invalidateAppQuery', () => {
    ...
    expect(QUERY_CLIENT.getQueryData(['/api/foo', 'POST', JSON.stringify({ id: 1, name: 'a' }), true])).toEqual({
