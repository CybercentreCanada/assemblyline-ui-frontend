...
import type { TooltipProps } from 'ui/text/Tooltip';
import { Tooltip } from 'ui/text/Tooltip';

export const InputCircularSkeleton = memo(() => {
...
export const InputFormButtonTooltip = memo(({ children }: { children?: TooltipProps['children'] }) => {
...
export const InputFormButton = memo(({ children, ...props }: ButtonProps) => {
  const [get] = usePropStore<InputControllerProps>();
...
export const InputButtonFormControlLabel = memo(({ children, label, ...props }: InputButtonFormControlLabelProps) => {
  const [get] = usePropStore<InputControllerProps>();
  ...
  return (
    <FormControlLabel
      control={loading ? <InputCircularSkeleton /> : (children as ReactElement)}
      disabled={loading || disabled || readOnly}
      label={label}
      {...props}
      slotProps={{
...
export const InputButtonLabel = memo(
  ({
