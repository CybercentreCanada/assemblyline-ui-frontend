...
import { useAppBar, useAppLayout, useAppUser } from '../app/hooks';
...
const AppTocElement: FC<AppTocElementProps> = ({ translation, item }) => {
...
const AppTocInternal = ({ children, translation, items, titleI18nKey = 'toc', topI18nKey = 'top' }: AppTocProps) => {
  const theme = useTheme();
  const { autoHide: autoHideAppbar } = useAppBar();
  const { current: currentLayout } = useAppLayout();
  const { t } = useTranslation([translation]);
  const { Link, location } = useAppRouter();
...
export const AppToc = memo(AppTocInternal);
